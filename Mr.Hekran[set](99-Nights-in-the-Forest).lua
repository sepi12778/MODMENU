...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Mr.Hekran_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local Mr.HekranFolder = "Mr.Hekran"
local ConfigurationFolder = Mr.HekranFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		Mr.HekranOpen = {Type = 'bind', Value = 'K', Name = 'Mr.Hekran Keybind'},
		-- buildwarnings
		-- Mr.Hekranprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.Mr.HekranOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(Mr.HekranFolder) then
				if isfile and isfile(Mr.HekranFolder..'/settings'..ConfigurationExtension) then
					file = readfile(Mr.HekranFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"Mr.HekranOpen":{"Value":"K","Type":"bind","Name":"Mr.Hekran Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Mr.Hekran Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Mr.Hekran had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "Mr.Hekran")
...
local Mr.Hekran = useStudio and script.Parent:FindFirstChild('Mr.Hekran') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local Mr.HekranDestroyed = false -- True when Mr.HekranLibrary:Destroy() is called

repeat
	if Mr.Hekran:FindFirstChild('Build') and Mr.Hekran.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Mr.Hekran | Build Mismatch')
		print('Mr.Hekran may encounter issues as you are running an incompatible interface version ('.. ((Mr.Hekran:FindFirstChild('Build') and Mr.Hekran.Build.Value) or 'No Build') ..').\n\nThis version of Mr.Hekran is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Mr.Hekran = Mr.Hekran, useStudio and script.Parent:FindFirstChild('Mr.Hekran') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Mr.Hekran.Enabled = false

if gethui then
	Mr.Hekran.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Mr.Hekran)
	Mr.Hekran.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Mr.Hekran.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Mr.Hekran.Name and Interface ~= Mr.Hekran then
			Interface.Enabled = false
			Interface.Name = "Mr.Hekran-Old"
		end
	end
elseif not useStudio then
...
if Mr.Hekran.AbsoluteSize.X < minSize.X and Mr.Hekran.AbsoluteSize.Y < minSize.Y then
...
local Main = Mr.Hekran.Main
local MPrompt = Mr.Hekran:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Mr.Hekran:FindFirstChild('Drag')
...
local Notifications = Mr.Hekran.Notifications

local SelectedTheme = Mr.HekranLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = Mr.HekranLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Mr.Hekran.Main.BackgroundColor3 = SelectedTheme.Background
	Mr.Hekran.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Mr.Hekran.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Mr.Hekran.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Mr.Hekran.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Mr.Hekran.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Mr.Hekran.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Mr.Hekran.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Mr.Hekran.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Mr.Hekran:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Mr.Hekran | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Mr.Hekran | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Mr.Hekran had an issue decoding the configuration file, please try delete the file and reopen Mr.Hekran.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(Mr.HekranLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Mr.Hekran | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--Mr.HekranLibrary:Notify({Title = "Mr.Hekran Flags", Content = "Mr.Hekran was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(Mr.HekranLibrary.Flags) do
...
function Mr.HekranLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Mr.Hekran | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			Mr.HekranLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			Mr.HekranLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "Mr.HekranOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(Mr.HekranFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Mr.Hekran Settings', 0, true)

	if TabList['Mr.Hekran Settings'] then
		TabList['Mr.Hekran Settings'].LayoutOrder = 1000
	end

	if Elements['Mr.Hekran Settings'] then
		Elements['Mr.Hekran Settings'].LayoutOrder = 1000
	end
...
function Mr.HekranLibrary:CreateWindow(Settings)
	print('creating window')
	if Mr.Hekran:FindFirstChild('Loading') then
		if getgenv and not getgenv().Mr.HekranCached then
			Mr.Hekran.Enabled = true
			Mr.Hekran.Loading.Visible = true

			task.wait(1.4)
			Mr.Hekran.Loading.Visible = false
		end
	end

	if getgenv then getgenv().Mr.HekranCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				Mr.HekranLibrary:Notify({Title = 'Build Mismatch', Content = 'Mr.Hekran may encounter issues as you are running an incompatible interface version ('.. ((Mr.Hekran:FindFirstChild('Build') and Mr.Hekran.Build.Value) or 'No Build') ..').\n\nThis version of Mr.Hekran is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "Mr.HekranOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "Mr.HekranOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(Mr.HekranFolder) then
		makefolder(Mr.HekranFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Mr.Hekran"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Mr.Hekran Interface Suite" then
		LoadingFrame.Version.Text = "Mr.Hekran UI"
	end
	...
	if not Settings.DisableMr.HekranPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				Mr.HekranLibrary:Notify({
					Title = "Mr.Hekran Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(Mr.HekranFolder.."/Discord Invites") then
			makefolder(Mr.HekranFolder.."/Discord Invites")
		end

		if isfile and not isfile(Mr.HekranFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(Mr.HekranFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Mr.Hekran RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(Mr.HekranFolder.."/Key System") then
			makefolder(Mr.HekranFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Mr.Hekran | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
				end
			end
		end
		...
		if isfile and isfile(Mr.HekranFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(Mr.HekranFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(Mr.HekranFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						Mr.HekranLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called Mr.HekranLibrary:Destroy()
				if Mr.HekranDestroyed then
					return
				end
				if not Success then
					...
					print("Mr.Hekran | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= Mr.HekranLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					Mr.HekranLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Mr.Hekran.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Mr.Hekran | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					Mr.HekranLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Mr.Hekran | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					Mr.HekranLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Mr.Hekran | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					Mr.HekranLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= Mr.HekranLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Mr.Hekran | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						Mr.HekranLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Mr.Hekran | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Mr.Hekran specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					Mr.HekranLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			Mr.HekranLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			Mr.HekranLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Mr.Hekran had an issue creating settings.') end
...
function Mr.HekranLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function Mr.HekranLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function Mr.HekranLibrary:Destroy()
	Mr.HekranDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Mr.Hekran Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "Mr.HekranOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function Mr.HekranLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				Mr.HekranLibrary:Notify({Title = "Mr.Hekran Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			Mr.HekranLibrary:Notify({Title = "Mr.Hekran Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Mr.Hekran Configurations Error | '..tostring(result))
			Mr.HekranLibrary:Notify({Title = "Mr.Hekran Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	Mr.HekranLibrary.LoadConfiguration()
